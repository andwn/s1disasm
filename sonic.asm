...
.equ EnableSRAM, 0	/* change to 1 to enable SRAM */
.equ BackupSRAM, 1
.equ AddressSRAM, 3	/* 0 = odd+even; 2 = even only; 3 = odd only */

.equ Revision, 0	/* change to 1 for JP1 revision */

.equ ZoneCount, 7	/* discrete zones are: GHZ, MZ, SYZ, LZ, SLZ, SBZ and Ending */

.equ OptimiseSound, 0	/* change to 1 to optimise sound queuing */

# ===========================================================================

StartOfRom:
Vectors:	dc.l 0xFFFE00, EntryPoint, BusError, AddressError
		...
		dc.l ErrorTrap,	ErrorTrap, ErrorTrap, ErrorTrap
		dc.l ErrorTrap,	ErrorTrap, ErrorTrap, ErrorTrap
Console:	.ascii "SEGA MEGA DRIVE " /* Hardware system ID */
Date:		.ascii "(C)SEGA 1991.APR" /* Release date */
Title_Local:	.ascii "SONIC THE               HEDGEHOG                " /* Domestic name */
Title_Int:	.ascii "SONIC THE               HEDGEHOG                " /* International name */
Serial:		.if Revision==0
		.ascii "GM 00001009-00"   /* Serial/version number */
		.else
			.ascii "GM 00004049-01"
		.endc
Checksum:	dc.w 0
		.ascii "J               " /* I/O support */
RomStartLoc:	dc.l StartOfRom		/* ROM start */
RomEndLoc:	dc.l EndOfRom-1		/* ROM end */
RamStartLoc:	dc.l 0xFF0000		/* RAM start */
RamEndLoc:	dc.l 0xFFFFFF		/* RAM end */
SRAMSupport:	.if EnableSRAM==1
		dc.b 0x52, 0x41, 0xA0+(BackupSRAM<<6)+(AddressSRAM<<3), 0x20
		.else
		dc.l 0x20202020
		.endc
		dc.l 0x20202020		/* SRAM start ($200001) */
		dc.l 0x20202020		/* SRAM end ($20xxxx) */
Notes:		.ascii "                                                    "
Region:		.ascii "JUE             " /* Region */
...
EntryPoint:
	...
	PortA_Ok:
		...
		movem.w	(a5)+,d5-d7
		movem.l	(a5)+,a0-a4
		move.b	-0x10FF(a1),d0	/* get hardware version (from $A10001) */
		andi.b	#0xF,d0
		beq.s	SkipSecurity
		move.l	#0x53454741,0x2F00(a1) /* move "SEGA" to TMSS register ($A14000) */
...
PSGInitLoop:
		move.b	(a5)+,0x11(a3)	/* reset	the PSG */
		dbf	d5,PSGInitLoop
		move.w	d0,(a2)
...
# ===========================================================================
SetupValues:	dc.w 0x8000		/* VDP register start number */
		dc.w 0x3FFF		/* size of RAM/4 */
		dc.w 0x100		/* VDP register diff */
		...
		dc.b 0x14		/* VDP $81 - Megadrive mode, DMA enable */
		dc.b (0xC000>>10)	/* VDP $82 - foreground nametable address */
		dc.b (0xF000>>10)	/* VDP $83 - window nametable address */
		dc.b (0xE000>>13)	/* VDP $84 - background nametable address */
		dc.b (0xD800>>9)		/* VDP $85 - sprite table address */
		...
		dc.b 0x81		/* VDP $8C - 40 cell display */
		dc.b (0xDC00>>10)	/* VDP $8D - hscroll table address */
		dc.b 0			/* VDP $8E - unused */
		dc.b 1			/* VDP $8F - VDP increment */
		dc.b 1			/* VDP $90 - 64 cell hscroll size */
		dc.b 0			/* VDP $91 - window h position */
		dc.b 0			/* VDP $92 - window v position */
		dc.w 0xFFFF		/* VDP $93/94 - DMA length */
		dc.w 0			/* VDP $95/96 - DMA source */
		dc.b 0x80		/* VDP $97 - DMA fill VRAM */
		dc.l 0x40000080		/* VRAM address 0 */

		dc.b 0xAF		/* xor	a */
		dc.b 0x01, 0xD9, 0x1F	/* ld	bc,1fd9h */
		dc.b 0x11, 0x27, 0x00	/* ld	de,0027h */
		dc.b 0x21, 0x26, 0x00	/* ld	hl,0026h */
		dc.b 0xF9		/* ld	sp,hl */
		dc.b 0x77		/* ld	(hl),a */
		dc.b 0xED, 0xB0		/* ldir */
		dc.b 0xDD, 0xE1		/* pop	ix */
		dc.b 0xFD, 0xE1		/* pop	iy */
		dc.b 0xED, 0x47		/* ld	i,a */
		dc.b 0xED, 0x4F		/* ld	r,a */
		dc.b 0xD1		/* pop	de */
		dc.b 0xE1		/* pop	hl */
		dc.b 0xF1		/* pop	af */
		dc.b 0x08		/* ex	af,af' */
		...
		dc.b 0xF3		/* di */
		dc.b 0xED, 0x56		/* im1 */
		dc.b 0x36, 0xE9		/* ld	(hl),e9h */
		dc.b 0xE9		/* jp	(hl) */

		dc.w 0x8104		/* VDP display mode */
		dc.w 0x8F02		/* VDP increment */
		dc.l 0xC0000000		/* CRAM write mode */
		dc.l 0x40000010		/* VSRAM address 0 */

		dc.b 0x9F, 0xBF, 0xDF, 0xFF	/* values for PSG channel volumes */
# ===========================================================================
...
CheckSumCheck:
	...
	2:
		move.l	d7,(a6)+
		dbf	d6,2b	/* clear RAM ($FE00-$FFFF) */

		move.b	(z80_version).l,d0
		andi.b	#0xC0,d0
		move.b	d0,(v_megadrive).w /* get region setting */
		move.l	#0x696e6974,(v_init).w /* set flag so checksum won't run again */
...
CheckSumError:
		bsr.w	VDPSetupGame
		move.l	#0xC0000000,(vdp_control_port).l /* set VDP to CRAM write */
		moveq	#0x3F,d7

	1:
		move.w	#cRed,(vdp_data_port).l /* fill palette with red */
		dbf	d7,1b	/* repeat $3F more times */
...
ChkInstr:
		move.b	#0xA,(v_errortype).w
		bra.s	loc_462

TrapvInstr:
		move.b	#0xC,(v_errortype).w
		bra.s	loc_462

PrivilegeViol:
		move.b	#0xE,(v_errortype).w
		bra.s	loc_462

Trace:
		move.b	#0x10,(v_errortype).w
		bra.s	loc_462

Line1010Emu:
		move.b	#0x12,(v_errortype).w
		addq.l	#2,2(sp)
		bra.s	loc_462

Line1111Emu:
		move.b	#0x14,(v_errortype).w
		addq.l	#2,2(sp)
		bra.s	loc_462
...
ShowErrorMessage:
	...
	1:
		...
		locVRAM	(vram_fg+0x604)
		moveq	#0x12,d1		/* number of characters (minus 1) */
	...
	2:
		...
		addi.w	#0x790,d0
		move.w	d0,(a6)
		dbf	d1,2b	/* repeat for number of characters */
		rts	
...
ShowErrorValue:
		move.w	#0x7CA,(a6)	/* display "$" symbol */
		moveq	#7,d2

	1:
		rol.l	#4,d0
		bsr.s	2f	/* display 8 numbers */
		dbf	d2,1b
		rts	
...
VBlank:
		...
		move.l	#0x40000010,(vdp_control_port).l
...
VBla_00:
		cmpi.b	#0x80+id_Level,(v_gamemode).w
		beq.s	1f
...
1:
	...
	2:
		...
		writeVRAM	v_sgfx_buffer,0x2E0,vram_sonic /* load new Sonic gfx */
		move.b	#0,(f_sonframechg).w

	3:
		startZ80
		movem.l	(v_screenposx).w,d0-d7
		movem.l	d0-d7,(0xFFFFFF10).w
		movem.l	(v_bgscroll1).w,d0-d1
		movem.l	d0-d1,(0xFFFFFF30).w
		cmpi.b	#96,(v_hbla_line).w
		bhs.s	Demo_Time
		move.b	#1,(0xFFFFF64F).w
		addq.l	#4,sp
		bra.w	VBla_Exit
...
VBla_0E:
		bsr.w	sub_106E
		addq.b	#1,(0xFFFFF628).w
		move.b	#0xE,(v_vbla_routine).w
		rts	
# ===========================================================================
...
HBlank:
		...
		move.l	#0xC0000000,4(a1) /* set VDP to CRAM write */
		...
		move.l	(a0)+,(a1)
		move.l	(a0)+,(a1)
		move.w	#0x8A00+223,4(a1) /* reset HBlank register */
		movem.l	(sp)+,a0-a1
		tst.b	(0xFFFFF64F).w
		bne.s	loc_119E
...
JoypadInit:
		...
		waitZ80
		moveq	#0x40,d0
		move.b	d0,(0xA10009).l	/* init port 1 (joypad 1) */
		move.b	d0,(0xA1000B).l	/* init port 2 (joypad 2) */
		move.b	d0,(0xA1000D).l	/* init port 3 (expansion) */
		startZ80
		rts	
...
ReadJoypads:
		...
		lea	(0xA10003).l,a1	/* first	joypad port */
		bsr.s	1f		/* do the first joypad */
...
VDPSetupGame:
	...
	1:
		move.w	(a2)+,(a0)
		dbf	d7,1b	/* set the VDP registers */

		move.w	(VDPSetupArray+2).l,d0
		move.w	d0,(v_vdp_buffer1).w
		move.w	#0x8A00+223,(v_hbla_hreg).w
		moveq	#0,d0
		move.l	#0xC0000000,(vdp_control_port).l /* set VDP to CRAM write */
		move.w	#0x3F,d7
	...
	3:
		...
		bne.s	3b	/* if yes, branch */

		move.w	#0x8F02,(a5)	/* set VDP increment size */
		move.l	(sp)+,d1
		rts	
# End of function VDPSetupGame

# ===========================================================================
VDPSetupArray:	dc.w 0x8004		/* 8-colour mode */
		dc.w 0x8134		/* enable V.interrupts, enable DMA */
		dc.w 0x8200+(vram_fg>>10) /* set foreground nametable address */
		dc.w 0x8300+(0xA000>>10)	/* set window nametable address */
		dc.w 0x8400+(vram_bg>>13) /* set background nametable address */
		dc.w 0x8500+(vram_sprites>>9) /* set sprite table address */
		dc.w 0x8600		/* unused */
		dc.w 0x8700		/* set background colour (palette entry 0) */
		dc.w 0x8800		/* unused */
		dc.w 0x8900		/* unused */
		dc.w 0x8A00		/* default H.interrupt register */
		dc.w 0x8B00		/* full-screen vertical scrolling */
		dc.w 0x8C81		/* 40-cell display mode */
		dc.w 0x8D00+(vram_hscroll>>10) /* set background hscroll address */
		dc.w 0x8E00		/* unused */
		dc.w 0x8F02		/* set VDP increment size */
		dc.w 0x9001		/* 64-cell hscroll size */
		dc.w 0x9100		/* window horizontal position */
		dc.w 0x9200		/* window vertical position */
...
ClearScreen:
		fillVRAM	0,0xFFF,vram_fg /* clear foreground namespace */
	...
	1:
		...
		move.w	#0x8F02,(a5)
		fillVRAM	0,0xFFF,vram_bg /* clear background namespace */
	...
	2:
		...
		moveq	#0,d0
		move.w	#(0x280/4),d1	/* This should be ($280/4)-1, leading to a slight bug (first bit of v_pal_water is cleared) */

	3:
		move.l	d0,(a1)+
		dbf	d1,3b /* clear sprite table (in RAM) */

		lea	(v_hscrolltablebuffer).w,a1
		moveq	#0,d0
		move.w	#(0x400/4),d1	/* This should be ($400/4)-1, leading to a slight bug (first bit of the Sonic object's RAM is cleared) */

	4:
		move.l	d0,(a1)+
		dbf	d1,4b /* clear hscroll table (in RAM) */
		rts	
...
2:
	...
	4:
		...
		dbf	d0,4b	/* repeat for length of PLC */
...
NewPLC:
	...
	2:
		move.l	(a1)+,(a2)+
		move.w	(a1)+,(a2)+	/* copy PLC to RAM */
		dbf	d0,2b	/* repeat for length of PLC */
...
loc_160E:
		...
		move.l	d0,(0xFFFFF6E4).w
		move.l	d0,(0xFFFFF6E8).w
		move.l	d0,(0xFFFFF6EC).w
		move.l	d5,(0xFFFFF6F0).w
		move.l	d6,(0xFFFFF6F4).w
...
sub_1642:
		tst.w	(f_plc_execute).w
		beq.w	locret_16DA
		move.w	#9,(0xFFFFF6FA).w
		moveq	#0,d0
		move.w	(0xFFFFF684).w,d0
		addi.w	#0x120,(0xFFFFF684).w
		bra.s	loc_1676
...
sub_165E:
		tst.w	(f_plc_execute).w
		beq.s	locret_16DA
		move.w	#3,(0xFFFFF6FA).w
		moveq	#0,d0
		move.w	(0xFFFFF684).w,d0
		addi.w	#0x60,(0xFFFFF684).w
...
loc_1676:
		...
		move.l	(0xFFFFF6E4).w,d0
		move.l	(0xFFFFF6E8).w,d1
		move.l	(0xFFFFF6EC).w,d2
		move.l	(0xFFFFF6F0).w,d5
		move.l	(0xFFFFF6F4).w,d6
		lea	(v_ngfx_buffer).w,a1
...
loc_16AA:
		...
		beq.s	loc_16DC
		subq.w	#1,(0xFFFFF6FA).w
		bne.s	loc_16AA
		move.l	a0,(v_plc_buffer).w
		move.l	a3,(v_ptrnemcode).w
		move.l	d0,(0xFFFFF6E4).w
		move.l	d1,(0xFFFFF6E8).w
		move.l	d2,(0xFFFFF6EC).w
		move.l	d5,(0xFFFFF6F0).w
		move.l	d6,(0xFFFFF6F4).w
...
PaletteFadeIn:
		move.w	#0x003F,(v_pfade_start).w /* set start position = 0; size = $40 */
...
PalFadeIn_Alt:				/* start position and size are already set */
	...
	1:
		move.w	d1,(a0)+
		dbf	d0,1b 	/* fill palette with black */

		move.w	#0x15,d4

	2:
		move.b	#0x12,(v_vbla_routine).w
		bsr.w	WaitForVBla
...
FadeIn_FromBlack:
	...
	1:
		...
		dbf	d0,1b	/* repeat for size of palette */
...
1:
		...
		beq.s	2f		/* if yes, branch */
		move.w	d3,d1
		addi.w	#0x200,d1	/* increase blue	value */
		cmp.w	d2,d1		/* has blue reached threshold level? */
		bhi.s	3f	/* if yes, branch */
...
3:
		move.w	d3,d1
		addi.w	#0x20,d1		/* increase green value */
		cmp.w	d2,d1
		bhi.s	4f
		move.w	d1,(a0)+	/* update palette */
		rts	
...
PaletteFadeOut:
		move.w	#0x003F,(v_pfade_start).w /* start position = 0; size = $40 */
		move.w	#0x15,d4
...
3:
		...
		andi.w	#0xE0,d1
		beq.s	4f
		subi.w	#0x20,(a0)+	/* decrease green value */
		rts	
# ===========================================================================
...
4:
		...
		andi.w	#0xE00,d1
		beq.s	2f
		subi.w	#0x200,(a0)+	/* decrease blue	value */
		rts	
# ===========================================================================
...
PaletteWhiteIn:
	...
	1:
		move.w	d1,(a0)+
		dbf	d0,1b 	/* fill palette with white */
...
1:
		...
		beq.s	2f
		move.w	d3,d1
		subi.w	#0x200,d1	/* decrease blue	value */
		blo.s	3f
		cmp.w	d2,d1
		blo.s	3f
		move.w	d1,(a0)+
...
3:
		move.w	d3,d1
		subi.w	#0x20,d1		/* decrease green value */
		blo.s	4f
		cmp.w	d2,d1
		blo.s	4f
		move.w	d1,(a0)+
...
3:
		...
		cmpi.w	#cGreen,d1
		beq.s	4f
		addi.w	#0x20,(a0)+	/* increase green value */
		rts	
# ===========================================================================
...
4:
		...
		cmpi.w	#cBlue,d1
		beq.s	2f
		addi.w	#0x200,(a0)+	/* increase blue	value */
		rts	
# ===========================================================================
...
GM_Sega:
		...
		move.w	#0x8700,(a6)	/* set background colour (palette entry 0) */
		move.w	#0x8B00,(a6)	/* full-screen vertical scrolling */
		clr.b	(f_wtr_state).w
		disable_ints
		...
		.else
			...
			bmi.s   1f
			copyTilemap	0xFF0A40,0xC53A,2,1 /* hide "TM" with a white rectangle */
		.endc

	1:
		moveq	#palid_SegaBG,d0
		bsr.w	PalLoad2	/* load Sega logo palette */
		move.w	#-0xA,(v_pcyc_num).w
		move.w	#0,(v_pcyc_time).w
		move.w	#0,(v_pal_buffer+0x12).w
		move.w	#0,(v_pal_buffer+0x10).w
		move.w	(v_vdp_buffer1).w,d0
		ori.b	#0x40,d0
		move.w	d0,(vdp_control_port).l
...
Sega_WaitPal:
		...
		move.b	#0x14,(v_vbla_routine).w
		bsr.w	WaitForVBla
		move.w	#0x1E,(v_demolength).w
...
GM_Title:
	...
	Tit_ClrPal:
		...
		bsr.w	PalLoad1
		move.b	#id_CreditsText,(v_objspace+0x80).w /* load "SONIC TEAM PRESENTS" object */
	...
	Tit_LoadText:
		...
		move.w	#0,(0xFFFFFFEA).w /* unused variable */
		...
		move.w	#0x178,(v_demolength).w /* run title screen for $178 frames */
		lea	(v_objspace+0x80).w,a1
		moveq	#0,d0
		move.w	#7,d1
	...
	Tit_ClrObj2:
		...
		move.b	#id_TitleSonic,(v_objspace+0x40).w /* load big Sonic object */
		move.b	#id_PSBTM,(v_objspace+0x80).w /* load "PRESS START BUTTON" object */

		.if Revision==0
		.else
			tst.b   (v_megadrive).w	/* is console Japanese? */
			bpl.s   1f		/* if yes, branch */
		.endc

		move.b	#id_PSBTM,(v_objspace+0xC0).w /* load "TM" object */
		move.b	#3,(v_objspace+0xC0+obFrame).w
	1:
		move.b	#id_PSBTM,(v_objspace+0x100).w /* load object which hides part of Sonic */
		move.b	#2,(v_objspace+0x100+obFrame).w
		jsr	(ExecuteObjects).l
		bsr.w	DeformLayers
		jsr	(BuildSprites).l
		moveq	#plcid_Main,d0
...
Tit_MainLoop:
		...
		cmpi.w	#0x1C00,d0	/* has Sonic object passed $1C00 on x-axis? */
...
LevelSelect:
		...
		cmpi.w	#0x14,d0		/* have you selected item $14 (sound test)? */
		bne.s	LevSel_Level_SS	/* if not, go to	Level/SS subroutine */
		move.w	(v_levselsound).w,d0
		addi.w	#0x80,d0
		tst.b	(f_creditscheat).w /* is Japanese Credits cheat on? */
		beq.s	LevSel_NoCheat	/* if not, branch */
		cmpi.w	#0x9F,d0		/* is sound $9F being played? */
		beq.s	LevSel_Ending	/* if yes, branch */
		cmpi.w	#0x9E,d0		/* is sound $9E being played? */
...
LevSel_Level_SS:
		...
		bmi.w	LevelSelect
		cmpi.w	#id_SS*0x100,d0	/* check	if level is 0700 (Special Stage) */
...
#		 correct level order
		...
		dc.w 0x8000		/* Sound Test */
		.align 2
# ---------------------------------------------------------------------------
# Level	select codes
# ---------------------------------------------------------------------------
LevSelCode_J:	.if Revision==0
		dc.b btnUp,btnDn,btnL,btnR,0,0xFF
		.else
		dc.b btnUp,btnDn,btnDn,btnDn,btnL,btnR,0,0xFF
		.endc
		.align 2

LevSelCode_US:	dc.b btnUp,btnDn,btnL,btnR,0,0xFF
		.align 2
# ===========================================================================
...
loc_3422:
		...
		cmpi.w	#0x600,d0	/* is level number 0600 (special	stage)? */
...
LevSel_UpDown:
		move.w	#0xB,(v_levseldelay).w /* reset time delay */
		...
		bhs.s	LevSel_Down
		moveq	#0x14,d0		/* if selection moves below 0, jump to selection	$14 */
...
LevSel_SndTest:
		cmpi.w	#0x14,(v_levselitem).w /* is item $14 selected? */
		...
		bhs.s	LevSel_Right
		moveq	#0x4F,d0		/* if sound test	moves below 0, set to $4F */
...
.equ	textpos, (0x40000000+((0xE210&0x3FFF)<<16)+((0xE210&0xC000)>>14))
#					 $E210 is a VRAM address

		lea	(LevelMenuText).l,a1
		lea	(vdp_data_port).l,a6
		move.l	#textpos,d4	/* text position on screen */
		move.w	#0xE680,d3	/* VRAM setting (4th palette, $680th tile) */
		moveq	#0x14,d1		/* number of lines of text */

	LevSel_DrawAll:
		move.l	d4,4(a6)
		bsr.w	LevSel_ChgLine	/* draw line of text */
		addi.l	#0x800000,d4	/* jump to next line */
		...
		add.w	d0,d1
		adda.w	d1,a1
		move.w	#0xC680,d3	/* VRAM setting (3rd palette, $680th tile) */
		move.l	d4,4(a6)
...
LevSel_DrawSnd:
		locVRAM	0xEC30		/* sound test position on screen */
		move.w	(v_levselsound).w,d0
		addi.w	#0x80,d0
		move.b	d0,d2
		lsr.b	#4,d0
...
LevSel_ChgLine:
		moveq	#0x17,d2		/* number of characters per line */
...
Level_ClrRam:
	...
	Level_ClrVars3:
		...
		move.w	#0x8200+(vram_fg>>10),(a6) /* set foreground nametable address */
		move.w	#0x8400+(vram_bg>>13),(a6) /* set background nametable address */
		move.w	#0x8500+(vram_sprites>>9),(a6) /* set sprite table address */
		move.w	#0x9001,(a6)		/* 64-cell hscroll size */
		move.w	#0x8004,(a6)		/* 8-colour mode */
		...
		move.w	#0x8014,(a6)	/* enable H-interrupts */
		moveq	#0,d0
		move.b	(v_act).w,d0
		add.w	d0,d0
...
Level_GetBgm:
	...
	Level_PlayBgm:
		...
		move.b	#id_TitleCard,(v_objspace+0x80).w /* load title card object */

Level_TtlCardLoop:
		move.b	#0xC,(v_vbla_routine).w
		bsr.w	WaitForVBla
		jsr	(ExecuteObjects).l
		jsr	(BuildSprites).l
		bsr.w	RunPLC
		move.w	(v_objspace+0x108).w,d0
		cmp.w	(v_objspace+0x130).w,d0 /* has title card sequence finished? */
	...
	Level_SkipTtlCard:
		...
		move.b	#id_HUD,(v_objspace+0x40).w /* load HUD object */
...
Level_ChkWater:
		...
		move.b	#id_WaterSurface,(v_objspace+0x780).w /* load water surface object */
		move.w	#0x60,(v_objspace+0x780+obX).w
		move.b	#id_WaterSurface,(v_objspace+0x7C0).w
		move.w	#0x120,(v_objspace+0x7C0+obX).w
...
Level_Delay:
	...
	Level_DelayLoop:
		...
		move.w	#0x202F,(v_pfade_start).w /* fade in 2nd, 3rd & 4th palette lines */
		bsr.w	PalFadeIn_Alt
		tst.w	(f_demo).w	/* is an ending sequence demo running? */
		bmi.s	Level_ClrCardArt /* if yes, branch */
		addq.b	#2,(v_objspace+0x80+obRoutine).w /* make title card move */
		addq.b	#4,(v_objspace+0xC0+obRoutine).w
		addq.b	#4,(v_objspace+0x100+obRoutine).w
		addq.b	#4,(v_objspace+0x140+obRoutine).w
...
Level_FadeDemo:
		move.w	#0x3C,(v_demolength).w
		move.w	#0x3F,(v_pfade_start).w
...
Sync1:
		...
		move.b	#0xB,(v_ani0_time).w /* reset timer */
...
GM_Special:
	...
	SS_ClrNemRam:
		...
		move.w	#0x40,(v_ssrotate).w /* set stage rotation speed */
...
loc_47D4:
	...
	SS_EndClrObjRam:
		...
		move.b	#id_SSResult,(v_objspace+0x5C0).w /* load results screen object */
...
loc_4A18:
		adda.w	#0xC,a1
		lea	(v_pal_dry+0x5A).w,a2
		cmpi.w	#0xA,d0
		blo.s	loc_4A2E
		subi.w	#0xA,d0
		lea	(v_pal_dry+0x7A).w,a2
...
# ===========================================================================
byte_4A3C:	dc.b 3,	0, 7, 0x92, 3, 0, 7, 0x90, 3, 0, 7, 0x8E, 3, 0, 7,	0x8C

		dc.b 3,	0, 7, 0x8B, 3, 0, 7, 0x80, 3, 0, 7, 0x82, 3, 0, 7,	0x84
		dc.b 3,	0, 7, 0x86, 3, 0, 7, 0x88, 7, 8, 7, 0, 7,	0xA, 7, 0xC
		dc.b 0xFF, 0xC, 7, 0x18, 0xFF, 0xC, 7, 0x18, 7, 0xA, 7, 0xC, 7,	8, 7, 0
		dc.b 3,	0, 6, 0x88, 3, 0, 6, 0x86, 3, 0, 6, 0x84, 3, 0, 6,	0x82
		dc.b 3,	0, 6, 0x81, 3, 0, 6, 0x8A, 3, 0, 6, 0x8C, 3, 0, 6,	0x8E
		dc.b 3,	0, 6, 0x90, 3, 0, 6, 0x92, 7, 2, 6, 0x24, 7, 4, 6,	0x30
		dc.b 0xFF, 6, 6,	0x3C, 0xFF, 6, 6,	0x3C, 7,	4, 6, 0x30, 7, 2, 6, 0x24
		.align 2
byte_4ABC:	dc.b 0x10, 1, 0x18, 0, 0x18, 1, 0x20, 0, 0x20, 1, 0x28, 0, 0x28, 1
		.align 2
...
SS_BGAnimate:
		move.w	(0xFFFFF7A0).w,d0
		bne.s	loc_4BF6
		move.w	#0,(v_bgscreenposy).w
		move.w	(v_bgscreenposy).w,(0xFFFFF618).w
...
# ===========================================================================
byte_4CB8:	dc.b 9,	0x28, 0x18, 0x10, 0x28, 0x18, 0x10, 0x30, 0x18,	8, 0x10,	0
		.align 2
byte_4CC4:	dc.b 6,	0x30, 0x30, 0x30, 0x28, 0x18, 0x18, 0x18
		.align 2
byte_4CCC:	dc.b 8,	2, 4, 0xFF, 2, 3, 8, 0xFF, 4, 2, 2, 3, 8,	0xFD, 4,	2, 2, 3, 2, 0xFF
		.align 2
...
GM_Continue:
	...
	Cont_ClrObjRam:
		...
		move.l	#0x1000000,(v_screenposy).w
		move.b	#id_ContSonic,(v_player).w /* load Sonic object */
		move.b	#id_ContScrItem,(v_objspace+0x40).w /* load continue screen objects */
		move.b	#id_ContScrItem,(v_objspace+0x80).w
		move.b	#3,(v_objspace+0x80+obPriority).w
		move.b	#4,(v_objspace+0x80+obFrame).w
		move.b	#id_ContScrItem,(v_objspace+0xC0).w
		move.b	#4,(v_objspace+0xC0+obRoutine).w
		jsr	(ExecuteObjects).l
		jsr	(BuildSprites).l
		move.w	(v_vdp_buffer1).w,d0
		ori.b	#0x40,d0
...
loc_4DF2:
		jsr	(ExecuteObjects).l
		jsr	(BuildSprites).l
		cmpi.w	#0x180,(v_player+obX).w /* has Sonic run off screen? */
...
End_LoadData:
		...
		lea	(0xFFFF9400).w,a1 /* RAM address to buffer the patterns */
		bsr.w	KosDec
		moveq	#palid_Sonic,d0
		bsr.w	PalLoad1	/* load Sonic's palette */
...
End_LoadSonic:
		...
		move.w	#0xF800,(v_player+obInertia).w /* set Sonic's speed */
		move.b	#id_HUD,(v_objspace+0x40).w /* load HUD object */
		jsr	(ObjPosLoad).l
		jsr	(ExecuteObjects).l
		jsr	(BuildSprites).l
...
End_MoveSonic:
		...
		bne.s	End_MoveSon2
		cmpi.w	#0x90,(v_player+obX).w /* has Sonic passed $90 on x-axis? */
...
End_MoveSon2:
		subq.b	#2,d0
		bne.s	End_MoveSon3
		cmpi.w	#0xA0,(v_player+obX).w /* has Sonic passed $A0 on x-axis? */
		...
		move.b	#0x81,(f_lockmulti).w /* lock controls & position */
...
GM_Credits:
	...
	Cred_ClrPal:
		...
		moveq	#palid_Sonic,d0
		bsr.w	PalLoad1	/* load Sonic's palette */
		move.b	#id_CreditsText,(v_objspace+0x80).w /* load credits object */
		jsr	(ExecuteObjects).l
		jsr	(BuildSprites).l
...
EndingDemoLoad:
		...
		move.w	#0x8001,(f_demo).w /* set demo+ending mode */
...
EndDemo_LampVar:
		...
		dc.w 0xA00, 0x62C		/* x/y-axis position */
		dc.w 13			/* rings */
		dc.l 0			/* time */
		dc.b 0,	0		/* dynamic level event routine counter */
		dc.w 0x800		/* level bottom boundary */
		dc.w 0x957, 0x5CC		/* x/y axis screen position */
		dc.w 0x4AB, 0x3A6, 0, 0x28C, 0, 0 /* scroll info */
		dc.w 0x308		/* water height */
...
TryAgainEnd:
	...
	TryAg_ClrPal:
		...
		moveq	#palid_Ending,d0
		bsr.w	PalLoad1	/* load ending palette */
		clr.w	(v_pal_dry_dup+0x40).w
		move.b	#id_EndEggman,(v_objspace+0x80).w /* load Eggman object */
		jsr	(ExecuteObjects).l
		jsr	(BuildSprites).l
...
locret_6AD6:
	...
	locj_6DF2:
			rts
	locj_6DF4:
			dc.b 0x00,0x00,0x00,0x00,0x00,0x06,0x06,0x06,0x06,0x06,0x06,0x06,0x06,0x06,0x06,0x04
			dc.b 0x04,0x04,0x04,0x04,0x04,0x04,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02
			dc.b 0x02,0x00						
#===============================================================================
	Draw_SBz:
			moveq	#-0x10,d4
...
#===============================================================================
	...
	locj_6EF0:
			rts
	locj_6EF2:
			dc.b 0x00,0x00,0x00,0x00,0x00,0x00,0x06,0x06,0x04,0x04,0x04,0x04,0x04,0x04,0x04,0x04
			dc.b 0x04,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02
...
#===============================================================================			
	locj_6FE4:
			dc.b 0xFF,0x18,0xFF,0x18,0xFF,0x20,0xFF,0x28
	locj_6FEC:
			moveq	#0xF,d6
			move.l	#0x800000,d7
	locj_6FF4:			
...
DrawFlipY:
		...
		move.l	(a1)+,d5
		move.l	(a1)+,d4
		eori.l	#0x10001000,d4
		move.l	d4,(a6)
		add.l	d7,d0
		move.l	d0,(a5)
		eori.l	#0x10001000,d5
		move.l	d5,(a6)
		rts	
# ===========================================================================
...
DrawFlipXY:
		...
		move.l	(a1)+,d5
		move.l	(a1)+,d4
		eori.l	#0x18001800,d4
		swap	d4
		move.l	d4,(a6)
		add.l	d7,d0
		move.l	d0,(a5)
		eori.l	#0x18001800,d5
		swap	d5
		move.l	d5,(a6)
		rts	
...
# End of function DrawChunks
	...
	locj_7224:			
			...
			rts
	locj_724a:
			dc.b 0x00,0x00,0x00,0x00,0x06,0x06,0x06,0x04,0x04,0x04,0x00,0x00,0x00,0x00,0x00,0x00
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
	locj_72B2:
			dc.b 0xF7,0x08,0xF7,0x08,0xF7,0x10,0xF7,0x18
	locj_72Ba:
			lsr.w	#4,d0
			move.b	(a0,d0),d0
...
LevelDataLoad:
	...
	3:
		...
		moveq	#0,d0
		move.b	(a2),d0
		beq.s	4f	/* if 2nd PLC is 0 (i.e. the ending sequence), branch */
...
LevLoad_ClrRam:
		...
		lea	(v_lvllayout+0x40).w,a3 /* RAM address for background layout */
		moveq	#2,d1
...
# ---------------------------------------------------------------------------
CFlo_Data1:	dc.b 0x1C, 0x18, 0x14, 0x10, 0x1A, 0x16, 0x12,	0xE, 0xA,	6, 0x18,	0x14, 0x10, 0xC, 8, 4
		dc.b 0x16, 0x12, 0xE, 0xA, 6, 2, 0x14, 0x10, 0xC, 0
CFlo_Data2:	dc.b 0x1E, 0x16, 0xE, 6, 0x1A, 0x12,	0xA, 2
CFlo_Data3:	dc.b 0x16, 0x1E, 0x1A, 0x12, 6, 0xE,	0xA, 2
...
M_Card_GHZ:	dc.b 9 			/* GREEN HILL */
		dc.b 0xF8, 5, 0,	0x18, 0xB4
		dc.b 0xF8, 5, 0,	0x3A, 0xC4
		dc.b 0xF8, 5, 0,	0x10, 0xD4
		dc.b 0xF8, 5, 0,	0x10, 0xE4
		dc.b 0xF8, 5, 0,	0x2E, 0xF4
		dc.b 0xF8, 5, 0,	0x1C, 0x14
		dc.b 0xF8, 1, 0,	0x20, 0x24
		dc.b 0xF8, 5, 0,	0x26, 0x2C
		dc.b 0xF8, 5, 0,	0x26, 0x3C
		.align 2
M_Card_LZ:	dc.b 9			/* LABYRINTH */
		dc.b 0xF8, 5, 0,	0x26, 0xBC
		dc.b 0xF8, 5, 0,	0, 0xCC
		dc.b 0xF8, 5, 0,	4, 0xDC
		dc.b 0xF8, 5, 0,	0x4A, 0xEC
		dc.b 0xF8, 5, 0,	0x3A, 0xFC
		dc.b 0xF8, 1, 0,	0x20, 0xC
		dc.b 0xF8, 5, 0,	0x2E, 0x14
		dc.b 0xF8, 5, 0,	0x42, 0x24
		dc.b 0xF8, 5, 0,	0x1C, 0x34
		.align 2
M_Card_MZ:	dc.b 6			/* MARBLE */
		dc.b 0xF8, 5, 0,	0x2A, 0xCF
		dc.b 0xF8, 5, 0,	0, 0xE0
		dc.b 0xF8, 5, 0,	0x3A, 0xF0
		dc.b 0xF8, 5, 0,	4, 0
		dc.b 0xF8, 5, 0,	0x26, 0x10
		dc.b 0xF8, 5, 0,	0x10, 0x20
		.align 2
M_Card_SLZ:	dc.b 9			/* STAR LIGHT */
		dc.b 0xF8, 5, 0,	0x3E, 0xB4
		dc.b 0xF8, 5, 0,	0x42, 0xC4
		dc.b 0xF8, 5, 0,	0, 0xD4
		dc.b 0xF8, 5, 0,	0x3A, 0xE4
		dc.b 0xF8, 5, 0,	0x26, 4
		dc.b 0xF8, 1, 0,	0x20, 0x14
		dc.b 0xF8, 5, 0,	0x18, 0x1C
		dc.b 0xF8, 5, 0,	0x1C, 0x2C
		dc.b 0xF8, 5, 0,	0x42, 0x3C
		.align 2
M_Card_SYZ:	dc.b 0xA			/* SPRING YARD */
		dc.b 0xF8, 5, 0,	0x3E, 0xAC
		dc.b 0xF8, 5, 0,	0x36, 0xBC
		dc.b 0xF8, 5, 0,	0x3A, 0xCC
		dc.b 0xF8, 1, 0,	0x20, 0xDC
		dc.b 0xF8, 5, 0,	0x2E, 0xE4
		dc.b 0xF8, 5, 0,	0x18, 0xF4
		dc.b 0xF8, 5, 0,	0x4A, 0x14
		dc.b 0xF8, 5, 0,	0, 0x24
		dc.b 0xF8, 5, 0,	0x3A, 0x34
		dc.b 0xF8, 5, 0,	0xC, 0x44
		.align 2
M_Card_SBZ:	dc.b 0xA			/* SCRAP BRAIN */
		dc.b 0xF8, 5, 0,	0x3E, 0xAC
		dc.b 0xF8, 5, 0,	8, 0xBC
		dc.b 0xF8, 5, 0,	0x3A, 0xCC
		dc.b 0xF8, 5, 0,	0, 0xDC
		dc.b 0xF8, 5, 0,	0x36, 0xEC
		dc.b 0xF8, 5, 0,	4, 0xC
		dc.b 0xF8, 5, 0,	0x3A, 0x1C
		dc.b 0xF8, 5, 0,	0, 0x2C
		dc.b 0xF8, 1, 0,	0x20, 0x3C
		dc.b 0xF8, 5, 0,	0x2E, 0x44
		.align 2
M_Card_Zone:	dc.b 4			/* ZONE */
		dc.b 0xF8, 5, 0,	0x4E, 0xE0
		dc.b 0xF8, 5, 0,	0x32, 0xF0
		dc.b 0xF8, 5, 0,	0x2E, 0
		dc.b 0xF8, 5, 0,	0x10, 0x10
		.align 2
M_Card_Act1:	dc.b 2			/* ACT 1 */
		dc.b 4,	0xC, 0, 0x53, 0xEC
		dc.b 0xF4, 2, 0,	0x57, 0xC
M_Card_Act2:	dc.b 2			/* ACT 2 */
		dc.b 4,	0xC, 0, 0x53, 0xEC
		dc.b 0xF4, 6, 0,	0x5A, 8
M_Card_Act3:	dc.b 2			/* ACT 3 */
		dc.b 4,	0xC, 0, 0x53, 0xEC
		dc.b 0xF4, 6, 0,	0x60, 8
M_Card_Oval:	dc.b 0xD			/* Oval */
		dc.b 0xE4, 0xC, 0, 0x70, 0xF4
		dc.b 0xE4, 2, 0,	0x74, 0x14
		dc.b 0xEC, 4, 0,	0x77, 0xEC
		dc.b 0xF4, 5, 0,	0x79, 0xE4
		dc.b 0x14, 0xC, 0x18, 0x70,	0xEC
		dc.b 4,	2, 0x18,	0x74, 0xE4
		dc.b 0xC, 4, 0x18, 0x77, 4
		dc.b 0xFC, 5, 0x18, 0x79, 0xC
		dc.b 0xEC, 8, 0,	0x7D, 0xFC
		dc.b 0xF4, 0xC, 0, 0x7C, 0xF4
		dc.b 0xFC, 8, 0,	0x7C, 0xF4
		dc.b 4,	0xC, 0, 0x7C, 0xEC
		dc.b 0xC, 8, 0, 0x7C, 0xEC
		.align 2
M_Card_FZ:	dc.b 5			/* FINAL */
		dc.b 0xF8, 5, 0,	0x14, 0xDC
		dc.b 0xF8, 1, 0,	0x20, 0xEC
		dc.b 0xF8, 5, 0,	0x2E, 0xF4
		dc.b 0xF8, 5, 0,	0, 4
		dc.b 0xF8, 5, 0,	0x26, 0x14
		.align 2
...
M_Got_SonicHas:	dc.b 8			/* SONIC HAS */
		dc.b 0xF8, 5, 0,	0x3E, 0xB8
		dc.b 0xF8, 5, 0,	0x32, 0xC8
		dc.b 0xF8, 5, 0,	0x2E, 0xD8
		dc.b 0xF8, 1, 0,	0x20, 0xE8
		dc.b 0xF8, 5, 0,	8, 0xF0
		dc.b 0xF8, 5, 0,	0x1C, 0x10
		dc.b 0xF8, 5, 0,	0, 0x20
		dc.b 0xF8, 5, 0,	0x3E, 0x30
M_Got_Passed:	dc.b 6			/* PASSED */
		dc.b 0xF8, 5, 0,	0x36, 0xD0
		dc.b 0xF8, 5, 0,	0, 0xE0
		dc.b 0xF8, 5, 0,	0x3E, 0xF0
		dc.b 0xF8, 5, 0,	0x3E, 0
		dc.b 0xF8, 5, 0,	0x10, 0x10
		dc.b 0xF8, 5, 0,	0xC, 0x20
M_Got_Score:	dc.b 6			/* SCORE */
		dc.b 0xF8, 0xD, 1, 0x4A, 0xB0
		dc.b 0xF8, 1, 1,	0x62, 0xD0
		dc.b 0xF8, 9, 1,	0x64, 0x18
		dc.b 0xF8, 0xD, 1, 0x6A, 0x30
		dc.b 0xF7, 4, 0,	0x6E, 0xCD
		dc.b 0xFF, 4, 0x18, 0x6E, 0xCD
M_Got_TBonus:	dc.b 7			/* TIME BONUS */
		dc.b 0xF8, 0xD, 1, 0x5A, 0xB0
		dc.b 0xF8, 0xD, 0, 0x66, 0xD9
		dc.b 0xF8, 1, 1,	0x4A, 0xF9
		dc.b 0xF7, 4, 0,	0x6E, 0xF6
		dc.b 0xFF, 4, 0x18, 0x6E, 0xF6
		dc.b 0xF8, 0xD, 0xFF, 0xF0,	0x28
		dc.b 0xF8, 1, 1,	0x70, 0x48
M_Got_RBonus:	dc.b 7			/* RING BONUS */
		dc.b 0xF8, 0xD, 1, 0x52, 0xB0
		dc.b 0xF8, 0xD, 0, 0x66, 0xD9
		dc.b 0xF8, 1, 1,	0x4A, 0xF9
		dc.b 0xF7, 4, 0,	0x6E, 0xF6
		dc.b 0xFF, 4, 0x18, 0x6E, 0xF6
		dc.b 0xF8, 0xD, 0xFF, 0xF8,	0x28
		dc.b 0xF8, 1, 1,	0x70, 0x48
		.align 2
# ---------------------------------------------------------------------------
...
M_SSR_Chaos:	dc.b 0xD			/* "CHAOS EMERALDS" */
		dc.b 0xF8, 5, 0,	8, 0x90
		dc.b 0xF8, 5, 0,	0x1C, 0xA0
		dc.b 0xF8, 5, 0,	0, 0xB0
		dc.b 0xF8, 5, 0,	0x32, 0xC0
		dc.b 0xF8, 5, 0,	0x3E, 0xD0
		dc.b 0xF8, 5, 0,	0x10, 0xF0
		dc.b 0xF8, 5, 0,	0x2A, 0
		dc.b 0xF8, 5, 0,	0x10, 0x10
		dc.b 0xF8, 5, 0,	0x3A, 0x20
		dc.b 0xF8, 5, 0,	0, 0x30
		dc.b 0xF8, 5, 0,	0x26, 0x40
		dc.b 0xF8, 5, 0,	0xC, 0x50
		dc.b 0xF8, 5, 0,	0x3E, 0x60
...
byte_CD0D:	dc.b 7
		...
		dc.b 0xF8, 1, 1,	0x70, 0x48
byte_CD31:	dc.b 4
		dc.b 0xF8, 0xD, 0xFF, 0xD1,	0xB0
		dc.b 0xF8, 0xD, 0xFF, 0xD9,	0xD0
		dc.b 0xF8, 1, 0xFF, 0xE1, 0xF0
		dc.b 0xF8, 6, 0x1F, 0xE3, 0x40
byte_CD46:	dc.b 4
		dc.b 0xF8, 0xD, 0xFF, 0xD1,	0xB0
		dc.b 0xF8, 0xD, 0xFF, 0xD9,	0xD0
		dc.b 0xF8, 1, 0xFF, 0xE1, 0xF0
		dc.b 0xF8, 6, 0x1F, 0xE9, 0x40
byte_CD5B:	dc.b 3
		dc.b 0xF8, 0xD, 0xFF, 0xD1,	0xB0
		dc.b 0xF8, 0xD, 0xFF, 0xD9,	0xD0
		dc.b 0xF8, 1, 0xFF, 0xE1, 0xF0
byte_CD6B:	dc.b 0xC			/* "SPECIAL STAGE" */
		dc.b 0xF8, 5, 0,	0x3E, 0x9C
		dc.b 0xF8, 5, 0,	0x36, 0xAC
		dc.b 0xF8, 5, 0,	0x10, 0xBC
		dc.b 0xF8, 5, 0,	8, 0xCC
		dc.b 0xF8, 1, 0,	0x20, 0xDC
		dc.b 0xF8, 5, 0,	0, 0xE4
		dc.b 0xF8, 5, 0,	0x26, 0xF4
		dc.b 0xF8, 5, 0,	0x3E, 0x14
		dc.b 0xF8, 5, 0,	0x42, 0x24
		dc.b 0xF8, 5, 0,	0, 0x34
		dc.b 0xF8, 5, 0,	0x18, 0x44
		dc.b 0xF8, 5, 0,	0x10, 0x54
byte_CDA8:	dc.b 0xF			/* "SONIC GOT THEM ALL" */
		dc.b 0xF8, 5, 0,	0x3E, 0x88
		dc.b 0xF8, 5, 0,	0x32, 0x98
		dc.b 0xF8, 5, 0,	0x2E, 0xA8
		dc.b 0xF8, 1, 0,	0x20, 0xB8
		dc.b 0xF8, 5, 0,	8, 0xC0
		dc.b 0xF8, 5, 0,	0x18, 0xD8
		dc.b 0xF8, 5, 0,	0x32, 0xE8
		dc.b 0xF8, 5, 0,	0x42, 0xF8
		dc.b 0xF8, 5, 0,	0x42, 0x10
		dc.b 0xF8, 5, 0,	0x1C, 0x20
		dc.b 0xF8, 5, 0,	0x10, 0x30
		dc.b 0xF8, 5, 0,	0x2A, 0x40
		dc.b 0xF8, 5, 0,	0, 0x58
		dc.b 0xF8, 5, 0,	0x26, 0x68
		dc.b 0xF8, 5, 0,	0x26, 0x78
		.align 2
...
#
Smash_FragSpd1:	dc.w 0x400, -0x500	/* x-move speed,	y-move speed */
		dc.w 0x600, -0x100
		dc.w 0x600, 0x100
		dc.w 0x400, 0x500
		dc.w 0x600, -0x600
...
Smash_FragSpd2:	dc.w -0x600, -0x600
		dc.w -0x800, -0x200
		dc.w -0x800, 0x200
		dc.w -0x600, 0x600
		dc.w -0x400, -0x500
...
BldSpr_ScrPos:	dc.l 0				/* blank */
		dc.l v_screenposx&0xFFFFFF	/* main screen x-position */
		dc.l v_bgscreenposx&0xFFFFFF	/* background x-position	1 */
		dc.l v_bg3screenposx&0xFFFFFF	/* background x-position	2 */
# ---------------------------------------------------------------------------
...
Sonic_Main:	/* Routine 0 */
		...
		move.w	#0x780,obGfx(a0)
		move.b	#2,obPriority(a0)
		move.b	#0x18,obActWid(a0)
		move.b	#4,obRender(a0)
		move.w	#0x600,(v_sonspeedmax).w /* Sonic's top speed */
		move.w	#0xC,(v_sonspeedacc).w /* Sonic's acceleration */
		move.w	#0x80,(v_sonspeeddec).w /* Sonic's deceleration */
...
loc_12C7E:
		...
		bsr.w	Sonic_Water
		move.b	(v_anglebuffer).w,0x36(a0)
		move.b	(0xFFFFF76A).w,0x37(a0)
...
word_164B2:	dc.w 0x10, 0xE80,	0xE14, 0x370, 0xEEF, 0x302,	0xEEF, 0x340, 0xE14, 0x3AE
word_164C6:	dc.w 0x10, 0xF80,	0xF14, 0x2E0, 0xFEF, 0x272,	0xFEF, 0x2B0, 0xF14, 0x31E
word_164DA:	dc.w 0x10, 0x1080, 0x1014,	0x270, 0x10EF, 0x202, 0x10EF, 0x240,	0x1014, 0x2AE
word_164EE:	dc.w 0x10, 0xF80,	0xF14, 0x570, 0xFEF, 0x502,	0xFEF, 0x540, 0xF14, 0x5AE
word_16502:	dc.w 0x10, 0x1B80, 0x1B14,	0x670, 0x1BEF, 0x602, 0x1BEF, 0x640,	0x1B14, 0x6AE
word_16516:	dc.w 0x10, 0x1C80, 0x1C14,	0x5E0, 0x1CEF, 0x572, 0x1CEF, 0x5B0,	0x1C14, 0x61E
# ===========================================================================
...
AddPoints:
	...
	1:
		...
		1:
			...
			blo.s   3f /* if not, branch */

			addi.l  #5000,(v_scorelife).w /* increase requirement by 50000 */
			tst.b   (v_megadrive).w
			bmi.s   3f /* branch if Mega Drive is Japanese */
...
ObjPos_End:	.incbin	"objpos/ending.bin"
		.align 2
ObjPos_Null:	dc.b 0xFF, 0xFF, 0, 0, 0,	0
