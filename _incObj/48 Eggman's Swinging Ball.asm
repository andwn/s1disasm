...
loc_17B60:
		...
		move.l	0x34(a0),0x34(a1)
		dbf	d1,GBall_MakeLinks /* repeat sequence 5 more times */

GBall_MakeBall:
		move.b	#8,obRoutine(a1)
		move.l	#Map_GBall,obMap(a1) /* load different mappings for final link */
		move.w	#0x43AA,obGfx(a1) /* use different graphics */
		move.b	#1,obFrame(a1)
		move.b	#5,obPriority(a1)
		move.b	#0x81,obColType(a1) /* make object hurt Sonic */
		rts	
# ===========================================================================

GBall_PosData:	dc.b 0,	0x10, 0x20, 0x30, 0x40, 0x60	/* y-position data for links and	giant ball */
...
sub_17C2A:
		movea.l	0x34(a0),a1
		addi.b	#0x20,obAniFrame(a0)
		bcc.s	loc_17C3C
		bchg	#0,obFrame(a0)

loc_17C3C:
		move.w	obX(a1),0x3A(a0)
		move.w	obY(a1),d0
		add.w	0x32(a0),d0
		move.w	d0,0x38(a0)
