...
.equ smash_speed, 0x30		/* Sonic's horizontal speed */
# ===========================================================================

Smash_Main:	/* Routine 0 */
		addq.b	#2,obRoutine(a0)
		move.l	#Map_Smash,obMap(a0)
		move.w	#0x450F,obGfx(a0)
...
1:
		...
		bne.s	2b	/* if not, branch */
		move.w	smash_speed(a0),d0
		bpl.s	3f
		neg.w	d0

	3:
		cmpi.w	#0x480,d0	/* is Sonic's speed $480 or higher? */
		bcs.s	2b	/* if not, branch */
...
Smash_FragMove:	/* Routine 4 */
		bsr.w	SpeedToPos
		addi.w	#0x70,obVelY(a0)	/* make fragment	fall faster */
		bsr.w	DisplaySprite
		tst.b	obRender(a0)
		bpl.w	DeleteObject
		rts	
