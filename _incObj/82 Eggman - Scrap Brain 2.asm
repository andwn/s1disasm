...
SEgg_Main:	/* Routine 0 */
		...
		move.w	#0x2160,obX(a0)
		move.w	#0x5A4,obY(a0)
		move.b	#0xF,obColType(a0)
		move.b	#0x10,obColProp(a0)
		bclr	#0,obStatus(a0)
		clr.b	ob2ndRout(a0)
...
SEgg_ChkSonic:
		...
		move.w	#180,0x3C(a0)	/* set delay to 3 seconds */
		move.b	#1,obAnim(a0)
...
SEgg_PreLeap:
		subq.w	#1,0x3C(a0)	/* subtract 1 from time delay */
...
SEgg_Leap:
		...
		bgt.s	loc_199D0
		bne.s	loc_1996A
		move.w	#-0xFC,obVelX(a0) /* make Eggman leap */
		move.w	#-0x3C0,obVelY(a0)

loc_1996A:
		cmpi.w	#0x2132,obX(a0)
		bgt.s	loc_19976
		clr.w	obVelX(a0)

loc_19976:
		addi.w	#0x24,obVelY(a0)
		tst.w	obVelY(a0)
		bmi.s	SEgg_FindBlocks
		cmpi.w	#0x595,obY(a0)
		bcs.s	SEgg_FindBlocks
		move.w	#0x5357,obSubtype(a0)
		cmpi.w	#0x59B,obY(a0)
		bcs.s	SEgg_FindBlocks
		move.w	#0x59B,obY(a0)
		clr.w	obVelY(a0)
