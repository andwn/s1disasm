...
.equ jun_frame, 0x34		/* current frame */
.equ jun_reverse, 0x36		/* flag set when switch is pressed */
.equ jun_switch, 0x38		/* which switch will reverse the disc */
# ===========================================================================
...
# ===========================================================================
	...
	2:
		...
		move.b	#0x10,obFrame(a1) /* use large circular sprite */

1:
		move.l	#Map_Jun,obMap(a1)
		move.w	#0x4348,obGfx(a1)
		ori.b	#4,obRender(a1)
		move.b	#0x38,obActWid(a1)
...
Jun_Release:	/* Routine 6 */
	...
	1:
		...
		lea	(v_player).w,a1
		move.w	#0,obVelX(a1)
		move.w	#0x800,obVelY(a1)
		cmpi.b	#4,d0
		beq.s	3f
		move.w	#0x800,obVelX(a1)
		move.w	#0x800,obVelY(a1)
...
junPos:		dc.b -0x20,    0, -0x1E,   0xE /* disc x-pos, Sonic x-pos, disc y-pos, Sonic y-pos */
		dc.b -0x18,  0x18,  -0xE,  0x1E
		dc.b    0,  0x20,   0xE,  0x1E
		dc.b  0x18,  0x18,  0x1E,   0xE
		dc.b  0x20,    0,  0x1E,  -0xE
		dc.b  0x18, -0x18,   0xE, -0x1E
		dc.b    0, -0x20,  -0xE, -0x1E
		dc.b -0x18, -0x18, -0x1E,  -0xE
