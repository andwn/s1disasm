...
.equ mblock_origX, 0x30
.equ mblock_origY, 0x32

MBlock_Var:	dc.b 0x10, 0		/* object width,	frame number */
		dc.b 0x20, 1
		dc.b 0x20, 2
		dc.b 0x40, 3
		dc.b 0x30, 4
...
MBlock_Main:	/* Routine 0 */
		...
		move.w	#0x43BC,obGfx(a0)
		move.b	#7,obHeight(a0)

loc_FE44:
		cmpi.b	#id_SBZ,(v_zone).w /* check if level is SBZ */
		bne.s	loc_FE60
		move.w	#0x22C0,obGfx(a0) /* SBZ specific code (object 5228) */
		cmpi.b	#0x28,obSubtype(a0) /* is object 5228 ? */
		beq.s	loc_FE60	/* if yes, branch */
		move.w	#0x4460,obGfx(a0) /* SBZ specific code (object 523x) */
...
MBlock_Type06:
		bsr.w	SpeedToPos
		addi.w	#0x18,obVelY(a0)	/* make the platform fall */
		bsr.w	ObjFloorDist
...
loc_10004:
		tst.w	0x36(a0)		/* is platform set to move back? */
		...
		move.w	#300,0x34(a0)	/* set time delay to 5 seconds */
		rts	
# ===========================================================================

MBlock_0A_Wait:
		subq.w	#1,0x34(a0)	/* subtract 1 from time delay */
		bne.s	locret_1002E	/* if time remains, branch */
		move.w	#1,0x36(a0)	/* set platform to move back to its original position */
