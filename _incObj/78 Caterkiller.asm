...
.equ cat_parent, 0x3C		/* address of parent object */
# ===========================================================================
...
Cat_Main:	/* Routine 0 */
		...
		move.w	#0x22B0,obGfx(a0)
		cmpi.b	#id_SBZ,(v_zone).w /* if level is SBZ, branch */
		beq.s	1f
		move.w	#0x24FF,obGfx(a0) /* MZ specific code */
...
Cat_Head:	/* Routine 2 */
	...
	3:
		move.b	#0xA,obRoutine(a0)	/* goto Cat_Delete next */
		rts	
# ===========================================================================
...
2:
		...
		move.b	#0x10,0x2A(a0)
		move.w	#-0xC0,obVelX(a0)
		move.w	#0x40,obInertia(a0)
		bchg	#4,0x2B(a0)
		bne.s	loc_16AFC
		clr.w	obVelX(a0)
		neg.w	obInertia(a0)
...
loc_16B02:
	...
	3:
		...
		andi.b	#0xF,cat_parent(a0)
		move.b	d1,0x2C(a0,d0.w)
...
4:
		...
		moveq	#0,d0
		move.b	cat_parent(a0),d0
		move.b	#0x80,0x2C(a0,d0.w)
		.else
			moveq	#0,d0
			move.b	cat_parent(a0),d0
			move.b	#0x80,0x2C(a0,d0)
			neg.w	obX+2(a0)
			beq.s	5f
			btst	#0,obStatus(a0)
			beq.s	5f
...
loc_16C0C:
		...
		move.b	d1,0x2C(a0,d0.w)
		.else
			move.b	d1,0x2C(a0,d0)
			neg.w	obX+2(a0)
			beq.s	locj_173E4
			btst	#0,obStatus(a0)
			beq.s	locj_173E4
			cmpi.w	#-0xC0,obVelX(a0)
			bne.s	locj_173E4
			subq.w	#1,obX(a0)
			addq.b	#1,cat_parent(a0)
			moveq	#0,d0
...
# ===========================================================================
Cat_FragSpeed:	dc.w -0x200, -0x180, 0x180, 0x200
# ===========================================================================
...
loc_16CAA:
		...
		move.b	#0xC,obRoutine(a0)
		andi.b	#0xF8,obFrame(a0)
