...
Eni_Loop:
		...
		move.w	d5,d1
		lsr.w	d7,d1
		andi.w	#0x7F,d1		/* get format list entry */
		move.w	d1,d2		/* and copy it */
		cmpi.w	#0x40,d1		/* is the high bit of the entry set? */
		bhs.s	1f
...
1:
		...
		andi.w	#0xF,d2		/* get repeat count */
		lsr.w	#4,d1
		add.w	d1,d1
...
# loc_1768:
EniDec_00:
1:		move.w	a2,(a1)+	/* copy incremental copy word */
...
# loc_1772:
EniDec_01:
1:		move.w	a4,(a1)+	/* copy literal copy word */
		dbf d2,1b	/* repeat */
		bra.s	Eni_Loop
# ===========================================================================
# loc_177A:
...
# loc_177E:
1:		move.w	d1,(a1)+	/* copy inline value */
		dbf d2,1b	/* repeat */
...
EniDec_111:
		...
		beq.s	EniDec_Done
# loc_17A8:
1:		bsr.w	EniDec_FetchInlineValue	/* fetch new inline value */
		move.w	d1,(a1)+	/* copy it */
		dbf d2,1b	/* and repeat */
...
1:
		...
		btst	d6,d5
		beq.s	2f
		addi.w	#0x4000,d3	/* set second palette line bit */
# loc_17FC:
2:
		add.b	d1,d1		/* is the low palette line bit set? */
		bcc.s	3f	/* if not, branch */
		subq.w	#1,d6
		btst	d6,d5
		beq.s	3f
		addi.w	#0x2000,d3	/* set first palette line bit */
# loc_180A:
3:
		...
		btst	d6,d5
		beq.s	4f
		ori.w	#0x1000,d3	/* set Y-flip bit */
# loc_1818:
4:
		add.b	d1,d1		/* is the horizontal flip flag set? */
		bcc.s	5f	/* if not, branch */
		subq.w	#1,d6
		btst	d6,d5
		beq.s	5f
		ori.w	#0x800,d3	/* set X-flip bit */
# loc_1826:
5:
		move.w	d5,d1
		move.w	d6,d7
		sub.w	a5,d7		/* subtract length in bits of inline copy value */
		bcc.s	6f	/* branch if a new word doesn't need to be read */
		move.w	d7,d6
		addi.w	#16,d6
...
# loc_1856:
6:
		beq.s	8f	/* if the word has been exactly exhausted, branch */
